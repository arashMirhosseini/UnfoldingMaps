...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(255, 0, 0));
		triangle(50, 97, 45, 109, 55, 109);
		fill(0, 0, 0);
		text("City Marker", 75, 103);
		
		fill(255, 255, 255);
		ellipse(50, 122, 12, 12);
		fill(0, 0, 0);
		...
		fill(0, 0, 255);
		ellipse(50, 187, 12, 12);
		fill(0, 0, 0);
		text("Intermediate", 75, 187);
		
		fill(255, 0, 0);
		ellipse(50, 204, 12, 12);
		fill(0, 0, 0);
		text("Deep", 75, 204);
	...
	private void printQuakes() 
	{
		...
		for (Marker m : countryMarkers) {
			int quakeCounter = 0;
			int ocean = 0;
			String countryName = (String)m.getProperty("name");
			for (Marker qm : quakeMarkers) {
				EarthquakeMarker em =  (EarthquakeMarker)qm;
				//LandQuakeMarker lq = (LandQuakeMarker)qm;
				if (em.isOnLand() && (String)qm.getProperty("country") == countryName) {
					quakeCounter++;
				}
				else if (!em.isOnLand()) {
					ocean++;
